...
class FullFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def sub_train(self, curr_epoch, world_size, rank):
        if not self.loader_len:
            self._steps_per_epoch = (
                            len(self._dataloader) // self._gradient_accumulation_steps) * self._all_range_len
            self.loader_len = len(self._dataloader)
        else:
            if not self.loader_len == len(self._dataloader):
                self._steps_per_epoch = self._steps_per_epoch - ((self.loader_len - len(self._dataloader)) // self._gradient_accumulation_steps)
        ...
        if self.resource_flag:
            if not self._sampler:
                raise '_sampler is not none'
            self._sampler.set_epoch(curr_epoch)
        else:
            if not self.back_sample:
                raise 'back_sample is not none'
        ...
        for idx, batch in enumerate(self._dataloader):
            ...
            if (idx + 1) % self._gradient_accumulation_steps == 0:
                ...
                if (
                    ...
                    log_dict = {
                        "loss": loss_to_log,
                        "perplexity": math.exp(loss_to_log),
                        "lr": get_lr(
                            (
                                self._optimizer
                                if not self._optimizer_in_bwd
            ...
            if self.global_step % self.cfg.save_per_step == 0 and self.global_step != 0:
